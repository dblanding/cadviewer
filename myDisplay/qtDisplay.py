...
class qtViewer3d(qtBaseViewer):
    ...
    def keyPressEvent(self, event):  # original
        code = event.key()
    ...
    def keyPressEvent(self, event):  # modified
        code = event.key()
        if code in self._key_map:
            self._key_map[code]()
        elif code in range(256):
            ...
            pass  # Keep quiet about pressing 'ctrl' key
    ...
    def mouseMoveEvent(self, evt):  # Original version
        pt = evt.pos()
    ...
    def mouseMoveEvent(self, evt):  # Modified version
        pt = evt.pos()
        buttons = int(evt.buttons())
        modifiers = evt.modifiers()
        # ROTATE
