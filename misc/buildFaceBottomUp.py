...
def testEdge():
    ...
    for edge in edgeList:
        hCurve, umin, umax = BRep_Tool.Curve(edge)
        curve = hCurve.GetObject() # type: Geom_Curve
        print 'umin = ', umin
        print 'umax = ', umax
        print 'Is Periodic? ', curve.IsPeriodic()
        print 'Shape: ', curve.Continuity()
        print 'first: ', curve.FirstParameter()
        print 'last: ', curve.LastParameter()
        vectr = curve.DN(0.0, 2)
        print 'curvature at u=0: ', vectr.Magnitude()
        
    seg = GC_MakeSegment(gp_Pnt(0,0,0), gp_Pnt(2,2,2))
    edg = BRepBuilderAPI_MakeEdge(seg.Value()).Edge()
    print type(edg)
    hCurve, umin, umax = BRep_Tool.Curve(edg)
    curve = hCurve.GetObject()
    print type(curve) # type: Geom_Curve
    print 'Is Periodic? ', curve.IsPeriodic()
    print 'Shape: ', curve.Continuity()
    print 'first: ', curve.FirstParameter()
    print 'last: ', curve.LastParameter()
...
if __name__ == '__main__':
    ...
    add_function_to_menu('Experimental', buildFaceBotUp)
    add_function_to_menu('Experimental', testEdge)
